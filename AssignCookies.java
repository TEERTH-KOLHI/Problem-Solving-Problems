...
class AssignCookies{
	public static void main(String...args){
		int Greediness[] = {1, 2, 3};
		int Cookies[] = {1, 1};
		int res = findContentChildren(Greediness, Cookies);
	...
	public static int findContentChildren(int[] g, int[] s){
		...
		int number_of_cookies = s.length;
		int number_of_children = g.length;
		int outPut_value = 0, cookie = 0;
		
		for(int i = 0; i < number_of_children && cookie < number_of_cookies;){
			if(s[cookie] >= g[i]){
				i++;
				outPut_value++;
			}
